...
frames = []
# Walk the folder containing the results files and
# compile a list of filenames
fileList = FileSystemTools.makeDataFileList(DATA_FILES_PATH)
for f in fileList:
    # Load the data from the csv file into a dataframe
    data = FileSystemTools.load_results_into_frame(f)
    frames.append(data)
...
# todo remove once cleaned up instructions and templates for definition file
# if len(fileList) > 0:
#     elections = []
#     for f in fileList:
#         elections.append(FileSystemTools.make_election_obj_from_file(f))
# else:
elections = FileSystemTools.make_election_objects_from_file(fileList[0])
...
for data in frames:
    ...
    for election in elections:
        if election.fieldName in data.columns:
